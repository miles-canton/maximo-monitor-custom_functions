...
# 导入自定义函数
try:
    from custom_MJ.functions import HelloWorld_MJ as FUNCTION_CLASS
except ImportError as e:
    print(f"❌ 导入函数失败: {e}")
    print("📋 排查步骤:")
    print("   1. 检查 custom_MJ/functions.py 文件是否存在")
    print("   2. 检查 custom_MJ/__init__.py 文件是否存在")
    print("   3. 检查 HelloWorld_MJ 类是否在 custom_MJ/functions.py 中定义")
    print("   4. 检查是否有语法错误: python -m py_compile custom_MJ/functions.py")
    raise
